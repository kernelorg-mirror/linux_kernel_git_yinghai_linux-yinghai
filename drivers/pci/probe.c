		 ...
		 int size, struct resource *parent_res)
{
	...
	while (parent) {
		...
		dev_printk(KERN_DEBUG, &parent->dev,
				"busn_res: %s %02x to %pR\n",
				(size > 0) ? "extended" : "shrunk",
				(size > 0) ? size : -size, res);
		parent = parent->parent;
	}
}

static void pci_bus_shrink_top(struct pci_bus *parent,
		 int size, struct resource *parent_res)
{
...
static int pci_bridge_probe_busn_res(struct pci_bus *bus,
			 ...
			 int needed_size, struct resource **p,
			 int *extended_size)
{
	int ret = -ENOMEM;
	...
	if (*p) {
		...
		if (extended_size)
			*extended_size = new_size - old_size;
	}
				...
				int secondary, int subordinate)
{
	...
	/* need to check with not scanned sibling bridges */
...
start_broken:
	...
	if ((secondary || subordinate) && !pcibios_assign_all_busses() &&
		...
		if (!pass) {
			if ((pcibios_assign_all_busses() && !dev->subordinate)
			    || broken)
				/* Temporarily disable forwarding of the
...
rescan:
		...
		if (extended_size < shrink_size)
			shrink_size = extended_size;
